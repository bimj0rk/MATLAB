...
[x21, t2] = impuls1(-17, 15, 0.001, -10);
[x22, t2] = impuls1(-17, 15, 0.001, -5);
[x23, t2] = impuls1(-17, 15, 0.001, 4);
[x24, t2] = impuls1(-17, 15, 0.001, 8);
x2 = 2*x21 + 0*x22 - 3*x23 + 1*x24;
plot(t2, x2), xlabel('Time'), ylabel('x2'), grid

conv11 = conv(x1, x1);
tconv11 = 2*t1(1):0.01:(2*t1(end));
plot(tconv11, conv11), xlabel('Time'), ylabel('x1*x1');
grid on;

conv12 = conv(x1, x2);
tconv12 = -(30+17):0.01:(34+15);
plot(tconv12, conv12), xlabel('Time'), ylabel('x1*x2');
grid on;